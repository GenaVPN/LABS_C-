...
class Car(pygame.sprite.Sprite):
    def __init__(self, dect: tuple[int, int]):
        super().__init__()
        self.original_image = random.choice(IMG_CAR)
        self.image = self.original_image
        self.rect = self.image.get_rect()
        self.rect.center = dect
...
class Player(Car):
    def __init__(self):
        super().__init__((WIDTH // 2, HEIGHT - 100))
        self.dist = 7
        self.original_image = pygame.image.load("image/player/Slice 9.png")
        self.image = self.original_image
...
while run:
    ...
    if main:
        ...
        timer+=1
        # Усложнение игры
        if score > 1 and score % 20 == 0 and dif >=-15:
            dif-=10
            speed_car += 1
            bg_speed +=1
            if score % 40 ==0:
